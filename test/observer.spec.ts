...
describe('DataLayerObserver unit tests', () => {
  ...
  it('a developer can programmatically observe an object by reference', (done) => {
    ...
    observer.registerTarget(target, [{ name: 'query', select: '$[(profileID)]' }], 'digitalData.user.profile[0]',
      undefined, undefined, (...data: any[]) => { changes = data; }, undefined, true);
  ...
  it('replayReadOnLoad does not dispatch when no handler used readOnLoad', () => {
    expectNoCalls(globalMock.console, 'log');
  ...
  it('replayReadOnLoad dispatches only handlers that registered with readOnLoad', () => {
    expectNoCalls(globalMock.console, 'log');
