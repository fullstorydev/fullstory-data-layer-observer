...
To experiment with DLO, begin by reviewing some examples.
Something that describes getting started quickly and links to pre-built rule sets (e.g. CEDDL).
...
By default, DLO will output any logging related messages to the browser’s JavaScript console.  For production environments, a custom log appender is preferable, and it will allow you to redirect logging to a centralized location.

Below is an example of adding a custom FullStory log appender. (Be aware of [rate limiting](https://help.fullstory.com/hc/en-us/articles/360020623234#custom-property-rate-limiting) if sending logs to FullStory; however, DLO only logs when errors occur.)  When a log event is generated by DLO, it will pass the event to the custom appender, which sends the information to FullStory as a custom event.
...
A custom log appender is registered similar to other options described in the Configuration section.  For details on writing a custom appender, see the `LogAppender` interface and `ConsoleAppender` class in logger.ts.
