...
export class PickOperator extends Operator<PickOperatorOptions> {
  ...
  validate() {
    ...
    if (!properties || Array.isArray(properties) && properties.length === 0) super.throwValidationError(option, OperatorValidationError.MISSING);
    if (typeof properties !== 'string' && !Array.isArray(properties)) super.throwValidationError(option, OperatorValidationError.UNSUPPORTED);
    if (typeof properties === 'string' && properties[0] === ',') super.throwValidationError(option, OperatorValidationError.MALFORMED, 'leading comma');
    if (typeof properties === 'string' && properties[properties.length - 1] === ',') super.throwValidationError(option, OperatorValidationError.MALFORMED, 'trailing comma');
  }
}
